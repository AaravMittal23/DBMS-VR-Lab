...
HAVING COUNT(*) > 2;</pre>
    ...
    <div id="guideModal1" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center;" onclick="if(event.target.id==='guideModal1') closeGuide1()">
      <div style="background: white; border-radius: 8px; max-width: 600px; max-height: 80vh; overflow-y: auto; padding: 32px; box-shadow: 0 20px 25px rgba(0,0,0,0.15);">
...
function startQuiz1Timer() {
  const randomDelay = Math.random() * 8000 + 15000; // 15-23 seconds
  console.log('Quiz1 Timer started, will show quiz in', Math.round(randomDelay/1000), 'seconds');
  quiz1Timer = setTimeout(() => {
    showRandomQuiz1();
    startQuiz1Timer(); // Restart timer for next quiz
  }, randomDelay);
}

function showRandomQuiz1() {
  if (Math.random() > 0.4) { // 60% chance to show quiz
    const randomIndex = Math.floor(Math.random() * quiz1Questions.length);
    const question = quiz1Questions[randomIndex];
    displayQuiz1(question);
  }
}

function displayQuiz1(question) {
  console.log('Displaying Quiz1:', question.question);
...
function openGuide1() {
  const modal = document.getElementById('guideModal1');
  if (modal) {
    modal.style.display = 'flex';
  }
}

function closeGuide1() {
  const modal = document.getElementById('guideModal1');
  if (modal) {
