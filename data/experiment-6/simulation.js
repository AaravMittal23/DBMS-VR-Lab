 ...
 *
 * IMPORTANT: Uses window.sim6_renderSimulation (not window.sim5_renderSimulation)
 * so it does not collide with experiment-3's simulation when both scripts
 * are cached by the browser in the same session.
 */

window.sim6_renderSimulation = function() {
  return `
    <span class="badge">Interactive Simulation</span>
    <div class="card">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <h2 style="margin:0">ER Diagram Design Simulation</h2>
        <button onclick="openGuide6()" style="background:var(--accent);color:white;padding:8px 16px;border:none;border-radius:4px;cursor:pointer;font-weight:600">📖 Guide</button>
      </div>
      ...
      <div style="display:flex;gap:12px;margin-bottom:24px;flex-wrap:wrap">
        <button onclick="sim6_showStep(1)" id="sim6_step1Btn" class="step-button active">Step 1: Entities &amp; Attributes</button>
        <button onclick="sim6_showStep(2)" id="sim6_step2Btn" class="step-button">Step 2: Relationships</button>
        <button onclick="sim6_showStep(3)" id="sim6_step3Btn" class="step-button">Step 3: Relational Mapping</button>
      </div>
    </div>

    <!-- Step 1 -->
    <div id="sim6_step1" class="simulation-step">
      <div class="card">
    ...
    <!-- Step 2 -->
    <div id="sim6_step2" class="simulation-step" style="display:none">
      <div class="card">
    ...
    <!-- Step 3 -->
    <div id="sim6_step3" class="simulation-step" style="display:none">
      <div class="card">
    ...
    <!-- Quiz Modal -->
    <div id="quizModal2" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:1001;align-items:center;justify-content:center" onclick="if(event.target.id==='quizModal2') closeQuiz6()">
      <div style="background:white;border-radius:8px;max-width:500px;padding:32px;box-shadow:0 20px 25px rgba(0,0,0,0.15)">
        ...
        <div id="quiz6Content" style="color:var(--text)"></div>
      </div>
    </div>`;
};

window.sim6_showStep = function(n) {
  [1,2,3].forEach(i => {
    const el  = document.getElementById(`sim6_step${i}`);
    const btn = document.getElementById(`sim6_step${i}Btn`);
    if (el)  el.style.display  = i === n ? 'block' : 'none';
    if (btn) btn.classList.toggle('active', i === n);
  });
};

window.openGuide6 = function() {
  const modal = document.getElementById('guideModal6');
  if (modal) {
    modal.style.display = 'flex';
  }
};

window.closeGuide6 = function() {
  const modal = document.getElementById('guideModal6');
  if (modal) {
...
function startQuiz6Timer() {
  const randomDelay = Math.random() * 8000 + 15000; // 15-23 seconds
  console.log('Quiz6 Timer started, will show quiz in', Math.round(randomDelay/1000), 'seconds');
  quiz6Timer = setTimeout(() => {
    showRandomQuiz6();
    startQuiz6Timer();
  }, randomDelay);
}

function showRandomQuiz6() {
  if (Math.random() > 0.4) { // 60% chance to show quiz
    const randomIndex = Math.floor(Math.random() * quiz6Questions.length);
    const question = quiz6Questions[randomIndex];
    displayQuiz6(question);
  }
}

function displayQuiz6(question) {
  console.log('Displaying Quiz6:', question.question);
  const content = document.getElementById('quiz6Content');
  if (!content) return;
...
function checkAnswer2(selected, correct, explanation) {
  ...
  html += '</div>';
  html += '<button onclick="closeQuiz6()" style="width:100%;padding:12px;background:var(--primary);color:white;border:none;border-radius:6px;cursor:pointer;font-weight:600;margin-top:16px">Got it!</button>';
...
window.initQuiz6 = function() {
  if (document.getElementById('sim6_step1')) {
