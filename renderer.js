  ...
  procedure(data) {
    ...
    data.steps.forEach((s, i) => {
      ...
      if (s.interactiveTask) {
        // Change from input validation to a "Spoon-fed" Key Concept highlight
        interactiveHtml = `
          <div class="interactive-task" style="margin-top: 20px; padding: 16px; background: var(--bg-color); border-left: 4px solid var(--primary); border-radius: 4px;">
            <p style="font-weight: 600; margin-bottom: 8px; color: var(--primary);">💡 Key Concept</p>
            <p style="margin: 0; color: var(--text);">${s.interactiveTask.prompt}</p>
            ${s.interactiveTask.expected ? `<pre style="margin-top: 12px; background: #1e293b; color: #10b981; padding: 12px; border-radius: 4px;">${s.interactiveTask.expected[0]}</pre>` : ''}
          </div>
        `;
      }
